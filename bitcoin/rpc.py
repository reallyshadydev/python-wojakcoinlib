...
class BaseProxy(object):
    ...
    def __init__(self,
        ...
        if service_url is None:
            # Figure out the path to the wojakcoin.conf file
            if btc_conf_file is None:
                if platform.system() == 'Darwin':
                    btc_conf_file = os.path.expanduser('~/Library/Application Support/Wojakcoin/')
                elif platform.system() == 'Windows':
                    btc_conf_file = os.path.join(os.environ['APPDATA'], 'Wojakcoin')
                else:
                    btc_conf_file = os.path.expanduser('~/.wojakcoin')
                btc_conf_file = os.path.join(btc_conf_file, 'wojakcoin.conf')

            # Wojakcoin Core accepts empty rpcuser, not specified in config
            conf = {'rpcuser': ""}

            # Extract contents of wojakcoin.conf to build service_url
            try:
            ...
            # Treat a missing config as though it were empty
